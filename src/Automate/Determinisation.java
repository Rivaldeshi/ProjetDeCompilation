...
/**
 * Cette classe de determiniser un automate pour pouvoire verifier apres si un mot apartient a un automate 
 * facilement  je vous avertir c'est n'est pas faciel
 * @author Rivaldes Hi
 */
...
public class Determinisation {
	...
	/**
	 * cette fonction nous retourne juste l'epsilone fermeture d'une liste d'etat facilement comme sa 
	...
	public static List<Etat> epsilonFermeture(Automate aut, List<Etat> etats) {
		...
		// c'est juste l'algorithme du coure faur=t lire page 78 on a juste mit la boucle
		for (Etat etat : etats) {
	...
	public static Automate Determiniser(Automate automat) throws ValidationException {
		...
		// juste metre l'etat intial dans une liste pour pouvoire apeler notre epsilon fermeture
		...
		while (!Detat.isEmpty()) {
			...
			for (String Sym : Constans.APHABET) {
				if (!Sym.equals(Constans.EPSILON)) {
					...
					// je chercher la transister du detats courant par raport al la lettre couranter
                   ...
                   // si non on la cree et on ajoute a la file
					if (contains) {						
						...
						// on verifi si la Detats contient une etat finale si oui l'etat cree sera final
						for(Etat et:u){
							if(et.isFinal()){
								Q.Etat.setIsFinal();
							}
